...
function MultilingualGreeting({ entered }: { entered: boolean }) {
  const GREETS = ["Hello.", "ನಮಸ್ಕಾರ.", "नमस्ते.", "హలో.", "வணக்கம்.", "Welcome."];
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setFade(true);
      setTimeout(() => { setI((n) => (n + 1) % GREETS.length); setFade(false); }, 350);
    }, 2800);
  ...
  return (
    <p className="hero-welcome" style={{ transitionDelay: entered ? "200ms" : "0ms" }}>
      <span className="hero-welcome-text" style={{ opacity: fade ? 0 : 1, filter: fade ? "blur(2px)" : "blur(0px)", transition: "opacity 0.3s ease, filter 0.3s ease" }}>
        {GREETS[i]}
      </span>
    </p>
  );
