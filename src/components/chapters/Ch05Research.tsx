        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
          ...
          <div className="lg:col-span-4">
            <div className="lg:sticky lg:top-24">
              ...
              <h2 className="font-display italic text-[clamp(2.2rem,4vw,3.2rem)] leading-[0.92] text-[#F7F4EF] mb-4">Publications</h2>
              <p className="text-[0.95rem] leading-relaxed text-[#A39A90] mb-8 max-w-[32ch]">
                Published research across AI in HR, workforce analytics, organizational behavior, and business strategy.
              </p>

              <div className="flex flex-col gap-2 mb-8">
                <a href={ORCID_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2.5 px-4 py-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-[#A6CE39]/30 transition-all duration-300 group">
                  <span className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: "#A6CE3920", color: "#7DA128" }}><OrcidLogo /></span>
                  <span className="text-[0.8rem] font-mono text-[#A39A90] group-hover:text-[#D6D1C9] transition-colors">0009-0005-2576-8731</span>
                </a>
                <a href={SSRN_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2.5 px-4 py-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-[#154A7A]/30 transition-all duration-300 group">
                  <span className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: "#154A7A15", color: "#154A7A" }}><SsrnLogo /></span>
              ...
              <div className="space-y-3.5 mb-8">
                {[
                  ...
                  <div key={s.label} className="flex items-baseline justify-between border-b border-white/[0.05] pb-3">
                    <span className="text-[0.78rem] font-mono tracking-[0.05em] text-[#7E756B]">{s.label}</span>
                    <span className="font-display text-[1.5rem] text-[#D97A32]">{s.value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {years.map((y) => (
                  <span key={y} className="px-3 py-1.5 rounded-full border border-white/[0.06] text-[0.7rem] font-mono tracking-[0.08em] text-[#7E756B]">{y}</span>
                ))}
              </div>
            </div>
          </div>
